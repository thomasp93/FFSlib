...
int File_Unlink(char *file) {
	...
	*(block+posCharStart)=dir->type; // copy the type of the inode
	posCharStart++;
	strcpy(block+posCharStart, dir->name); // copy the name
	posCharStart+=MAX_FILENAME_LEN;
	strcpy(block+posCharStart, dir->size); // copy the size
	posCharStart+=MAX_FILE_SIZE_LEN;
...
int Dir_Create(char *path) { // TODO: problema creazione cartella duplicata, creazione cartella con path errato
	Sector* sector = (Sector*) calloc(1, sizeof(Sector));
	Inode* dadInode = (Inode*) calloc(1, sizeof(Inode));
	...
	if (granfather != NULL) // root case /
	{
		...
		for(i=0; i<INDEX_SIZE-1; i++)
			dadInode->blocks[atoi(dadInode->size)+i] = buff[i]; // copye the index son into the dad's block
		...
		if(indexInodeDad+sizeof(Inode)<SECTOR_SIZE) // if the inode is less than size free into the inode
			for(i=0; i<strlen(inodeInfo); i++)
				sector->data[indexInodeDad+i] = inodeInfo[i];
		else
		{
	...
	if(indexInode+sizeof(Inode)<SECTOR_SIZE) // if the inode is less than size free into the inode
		for(i=0; i<strlen(inodeInfo); i++)
			sector->data[indexInode+i] = inodeInfo[i];
	else
	{
...
int Dir_Read(char *path, void *buffer, int size) {
	...
	char* block = (char*) calloc(1, BLOCK_SIZE);
	char* token = (char*) malloc(MAX_FILENAME_LEN);
	...
	token = strtok(path2, "/"); // divide the path
	indexInode = 0;
	...
	while (token != NULL) // finchè ho token
	{
		...
		i = 0;
		while (i<atoi(dadInode->size) && strcmp(sonInode->name, str)!=0) // while visit the all inode or found the directory
		{
		...
		if (strcmp(sonInode->name, str)!=0) // if not found the directory
		{
	...
	while (i*(INDEX_SIZE-1)<atoi(dadInode->size) && i*INDEX_SIZE<size) // visit all inode
	{
		...
		// read the son inode
		posCharStart=indexInode; // recalculate the index of inode into the inode table
		sonInode->type = block[posCharStart]; // read the type of inode
		posCharStart+=1; // add the char readden
		snprintf(sonInode->name, MAX_FILENAME_LEN, "%s", block+posCharStart); // read the name of file (directory)
		posCharStart+=MAX_FILENAME_LEN;
...
int Dir_Unlink(char *path) {
	...
	char* block;
	char* fatherpath;
	char* grandfatherpath;
	char* fathername;
	char* posFatherInode;
	char* posIndextodelete;
	int indexSonInode, indexSonSector, indexSonBlock, i, indexFatherInode, indexFatherBlock, indexFatherSector, noChar, intIndex, posCharStart, blockread;
	...
	*(strstr(fatherData, sonName)) = '\0';    //find the father path
	...
	if(sonPos ==  NULL) {			// the directory not exist
		...
		i=0;
		while(i<INDEX_SIZE){
			sonIndex[i] = *(sonPos+strlen(sonName)+i);
		}
	}
	...
	if(posFatherInode ==  NULL) {			// the directory not exist
		...
		i=0;
		while(i<INDEX_SIZE){
			fatherIndex[i] = *(posFatherInode+strlen(fathername)+i);
		}
	}
	...
	for(i=0; i<INDEX_SIZE; i++)											// delete father "link" to the son
		*(posIndextodelete+i) = dadInode->blocks[atoi(dadInode->size) + 1 - INDEX_SIZE + i];
